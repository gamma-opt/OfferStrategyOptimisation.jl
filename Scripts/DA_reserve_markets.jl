...
folder = "Results/"
run_path = "DA_reserve_markets_"*Dates.format(now(), "yyyymmdd-HHMM")
...
market_variables!(model, sets, include_ID = false);
...
optimal_reserve_bids = value.(model[:v])
optimal_reserve_commitments = value.(model[:r])
...
run_path = "DA_reserve_fixed_then_ID_"*Dates.format(now(), "yyyymmdd-HHMM")
path = path*run_path*"/"
isdir(path) || mkdir(path)
@info("path = "*path)
...
@constraint(model, fix_reserve_bids, model[:v] .== optimal_reserve_bids)
@constraint(model, fix_reserve_commitments, model[:r] .== optimal_reserve_commitments)
